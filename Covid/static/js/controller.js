...
function get_c2_data() {
    $.ajax({
        url:"/c2",
        success:function(data){
            ec_center_option.series[0].data = data.data //收到返回值data之后，把返回值里面的数据拿出来重新更改之前option里面具体的数据
            ec_center.setOption(ec_center_option) //重新setoption，更新图像
        },
        error:function(xhr,type,errorThrown){
...
setInterval(get_time, 1000) //一秒执行一次，动态更新time
setInterval(get_c1_data, 1000*10) //10秒执行一次，动态更新c1的数据
